...
class Transformer(nn.Module):
    ...
    def __init__(self, vocab_size: int, d_model: int, num_heads: int,
                num_layers: int, d_ff: int, max_seq_length: int, dropout: float = 0.1):
        super().__init__()
        
        self.encoder_embedding = TransformerEmbedding(vocab_size, d_model, max_seq_length, dropout)
        self.decoder_embedding = TransformerEmbedding(vocab_size, d_model, max_seq_length, dropout)
    ...
    def create_masks(self, src: torch.Tensor, tgt: torch.Tensor) -> tuple:
        ...
        subsequent_mask = torch.triu(torch.ones((1, seq_len, seq_len)), diagonal=1).eq(0)
